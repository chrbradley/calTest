...
export default class FixedHeightList extends React.Component {
    ...
    return months.map((month) => {
      ...
      return (
        <View key={key} style={styles.month}>
          ...
          {weeks.map((week) => this._renderWeek(week, monthName, year))}
        </View>
      )
    })
  }
  ...
  _renderRows () {
    return this.state.renderModel.map(row => {
      return (
        <View key={`${row.year}`} style={[styles.rowContainer, {top: row.position}]}>
          {this._renderYear(row)}
          <View style={styles.monthContainer}>
            {this._renderMonths(row)}
          </View>
        </View>
      )
    })
      ...
      <View style={{flex: 1}}>
        <View style={styles.container}>
          <ScrollView
            ...
            <View style={[styles.scrollContainer, {height: this.state.bodyHeight}]}>
              {this._renderRows()}
            </View>
          </ScrollView>
        </View>
